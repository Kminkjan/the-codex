...
**Picking a number for a new migration:** take the next number after the highest across (a) this directory, (b) the remote history, and (c) any in-flight branch or open PR that adds a migration. As of 2026-07-26 that next number is **0028** — 0027 (`fendwick_board_and_tidy`, board curation after session 8) is the highest here, and 0026 added that session's rows. `supabase migration list --linked` showed the directory and the remote history matching through 0025 before 0026 was added.
