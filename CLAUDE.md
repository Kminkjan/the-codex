...
There's no test *framework*, but there are eleven assertion harnesses in [scripts/](scripts/), each runnable as `npx tsx scripts/<name>.ts` and exiting non-zero on failure:
...
| `feedback-check.ts` | the pure derivations in [src/feedback.ts](src/feedback.ts) — the bug/idea board's reading order and vote fold. Silent when wrong: **status outranks votes** (else the most-wanted item sits on top forever wearing a "done" chip), ties need a **total order** (most reports have zero votes, so a stable sort would visibly reshuffle on every refetch), the fold is **set-valued**, and a null viewer is **never** a voter | yes (`check:feedback`) |
...
- **Reads / state**: [src/campaignContext.tsx](src/campaignContext.tsx) loads the full campaign on mount via parallel `.select()` calls across 14 tables, transforms rows with per-table mappers (`mapPerson`, `mapQuest`, etc. — snake_case → camelCase, `desc` → `desc`, FK columns like `location_id` → `location`), then subscribes to every table via Supabase realtime. Entity tables (people, locations, quests, goals, factions, items, lore, monsters, sessions) splice incremental changes in through `applyArrayChange`. Four surfaces — `connections`, `board_positions`, `party_notes`, and the `feedback`/`feedback_votes` pair — **refetch the whole table on any change** (see the handlers at the bottom of `CampaignProvider`); this is intentional for v1 simplicity and is the reason a single entity delete may trigger several full refetches. The feedback pair shares one debounce slot and refetches *both* tables on a change to either, because the client shape is one array with votes folded in. **"Who's at the table" is channel Presence, not a table** (issue #74, migration 0021 dropped `presence_users`): the same campaign channel `.track()`s `{ id, name, initials, color }` for signed-in named editors, `CampaignProvider` exposes the deduped roster as `presenceUsers` (`usePresence()`), and occupancy expires with the socket. Presence is occupancy only — the "session is live" fact stays the `campaigns.active_session_id` row.
...
### Feedback (bugs & ideas about the app itself)

The party reports bugs and asks for features **from inside the codex**, not from GitHub — players don't have accounts, and a public repo would turn every "the map thing is broken" into a permanent public artifact under a real name. [migration 0040](supabase/migrations/0040_feedback.sql) + pure derivations in [src/feedback.ts](src/feedback.ts) + the panel in [src/feedbackPanel.tsx](src/feedbackPanel.tsx), opened from the sidebar's last section. **Triage stays manual**: nothing here files a GitHub issue, and it must not — that would publish player-authored text to a public repo on a button they didn't understand.

Four things worth knowing before touching it:

- **Reports are append-only for players; `status` is the only mutable column and only the DM may move it.** Same contract as `party_notes`/`session_events`, so the composer is `<NoteComposer>` (never blur-to-save) and there is no player edit or delete. The DM's status control is what makes the board not-write-only — a reporter who watches an item move reports the next one.
- **Votes are ROWS, not a counter.** Every client write here is fire-and-forget with no transaction, so `votes = votes + 1` would silently drop concurrent votes; and a toggle needs to know who voted, which is a uniqueness key. Note the two identities differ on purpose: `feedback.author` is a display name (a byline, like every other author column), `feedback_votes.user_id` is a uuid (nothing renders *who* voted — only the count and whether *you* did).
- **Prioritization is deliberately thin and should stay that way.** One bit per person, no weights, ranks, downvotes or threads. Five people who talk weekly don't need a decision procedure; a 3-2 split is noise. The order in `orderFeedback` is a reading order, not a verdict.
- **0040 is the first table here to set `replica identity full`**, and it isn't boilerplate: a filtered realtime subscription can't match `campaign_id` on a DELETE that publishes only the primary key, and un-voting *is* a delete. Every other table gets away with the default because its deletes are either absent or keyed by an id the client already holds.
